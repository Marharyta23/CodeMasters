...
import { startOfMonth, endOfMonth, eachDayOfInterval, format, getDate, isToday, addMonths, isWithinInterval } from "date-fns";
...
import { selectWaterDataDay, selectWaterDataMonth, selectDay } from "../../redux/water/selectors";
        ...
        <div className={css.monthlyInfo__waterList}>
            <div className={css.monthlyInfo__HeaderCalendar}>
                ...
                <div className={css.monthlyInfo__Pagination}>
                    <button onClick={handlePrevMonth} className={css.monthlyInfo__ButtonPagination}>
                    ...
                    <span className={css.monthlyInfo__CurrentMonth}>{format(currentDate, "MMMM yyyy")}</span>
                    <button onClick={handleNextMonth} disabled={isCurrentMonth} className={css.monthlyInfo__ButtonPagination}>
                        <IconWrapper className={css.monthlyInfo__iconWrapper} isCurrentMonth={isCurrentMonth}>
                            <svg>
                ...
                {formattedDays.map((item) => {
                    ...
                    return (
                        ...
                        >
                            ...
                            <Day className={css.monthlyInfo__Day} percentage={0} isToday={isToday(item.fullDate)}>
                                {item.day}
                            </Day>
                            {/* {dayInfo.percentage} */} {/*todo*/}
                            <p className={css.monthlyInfo__Percentage}>{waterPercentage[item.day]}%</p>
                        </div>
                    );
                })}
            </Days>
